...
- **Public stream token for smart home devices**: `/stream-${PUBLIC_STREAM_TOKEN}` is a secondary stream path that bypasses HTTP Basic Auth, intended for smart TV entry and TuneIn registration. It uses `auth_basic off` + `limit_conn stream_conn 10` (max 10 simultaneous connections per IP). If `PUBLIC_STREAM_TOKEN` is unset, the path resolves to `/stream-` which is harmless. Treat the token URL like a password — share person-to-person only.
