...
- **yt-dlp + Deno + remote components**: yt-dlp requires Deno (installed in `api/Dockerfile`) and the `--remote-components ejs:github` flag to solve YouTube's JS challenge. Without it, yt-dlp can authenticate but cannot unlock audio formats (returns "Only images are available for download"). **Note (Mar 2026)**: the ejs solver is currently broken for all YouTube player JS versions; the workaround is the `android_vr` player client (see below).
- **YouTube downloads blocked on AWS**: yt-dlp gets "Sign in to confirm you're not a bot" from AWS datacenter IPs. Current fix (Mar 2026): use `android_vr` player client (`--extractor-args youtube:player_client=android_vr`) + bgutil-provider 1.3.1+ for po_token generation + yt-dlp nightly (`pip install --pre --upgrade yt-dlp` in Dockerfile, since `android_vr` was broken in stable 2026.03.03). Cookies are intentionally NOT passed — `android_vr` doesn't support them and bgutil po_token handles bot detection. Once yt-dlp stable ships an ejs fix, revert: remove `--pre` from Dockerfile and remove the `player_client` extractor arg.
